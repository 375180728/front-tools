 ...
 */
export function arabia2SimplifiedChinese(Num) {
 ...
 * 壹亿贰仟叁佰肆拾伍万陆仟柒佰捌拾玖元整
 */
export function arabia2TraditionalChinese(Num) {
  for (var i = Num.length - 1; i >= 0; i--) {
    Num = Num.replace(',', ''); //替换Num中的“,”
    Num = Num.replace(' ', ''); //替换Num中的空格
  }
...
/**
 * HTML5 消息
 * @author ammo 2018-05-29
 * @param  {[type]} title   [description]
 * @param  {[type]} message [description]
 * @return {[type]}         [description]
 */
export function notifiyDesk(title, message) {
  var title = title || '新的消息';
  var options = options || {
    body: message || '默认消息'
    // icon: prize
  };
  ...
  Notification.requestPermission(function(permission) {
    if (permission === 'granted') {
      var notifycation = new Notification(title, options);
...
/**
 * 返回钱格式
 * @author ammo 2018-06-12
 * @param  {[type]} money [description]
 * @return {[type]}       [description]
 */
export function moneyFormat(money, places = 2) {
  return Number(money)
    .toFixed(places)
    .replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
}
